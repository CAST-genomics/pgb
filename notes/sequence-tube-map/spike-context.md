> **Migrated from [`CAST-genomics/sequence-tube-map-spike`](https://github.com/CAST-genomics/sequence-tube-map-spike), 2026-08-20.** This is the spike's CONTEXT.md,
> left as it was written. It is **descriptive** here and normative nowhere: PGB's
> [`CONTEXT.md`](../../CONTEXT.md) is the vocabulary and [`docs/adr/`](../../docs/adr) holds the
> decisions. Where this file and one of those disagree, this one is the historical record.
> See [`README.md`](README.md) for what came across and why.
...
| **segment** | One of the ~75 stretches of genomic sequence *inside* the tube map — every strand crossing one carries its bases. Drawn in `<g class="node">` with ids like `79337767`. The contents. Called "node" by the SVG and by upstream sequence-tube-map; renamed here to keep the two scales distinct. Full entry: [`CONTEXT.md`](../../CONTEXT.md) §segment. |
