# Reference genome - igv.js

> Source: [igv.org/doc/igvjs/Reference-Genome](https://igv.org/doc/igvjs/Reference-Genome)
...
IGV hosts a limited number of genome assemblies, which are listed in the table below. The UCSC GenArk site hosts an extensive list of assembly hubs, currently numbering 4,247. The complete listing can be found [here](https://hgdownload.soe.ucsc.edu/hubs/UCSC_GI.assemblyHubList.txt).
...
1. **UCSC two bit sequence file** (see [twobit](https://genome.ucsc.edu/goldenPath/help/twoBit.html)): `twoBitURL`
2. **BGZIPPED compressed fasta file**: `fastaURL`, `indexURL`, `compressedIndexURL`
3. **Indexed fasta file**: `fastaURL`, `indexURL`
4. **Fasta file without index** (useful for small (< 10 MB) sequences): `fastaURL`
...
| aliasURL | URL to a tab-delimited file defining aliases for chromosome names. The format is tab delimited with 1 line per chromosome. Each line begins with the canonical chromosome followed by aliases. See the example below. Optional | |
...
| tracks | A list of tracks to be loaded with the genome. See the [tracks](https://igv.org/doc/tracks/Tracks/) description for details. Optional | |
