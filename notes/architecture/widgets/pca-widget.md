...
2. **Lights up the corresponding dots** in the companion 2D PCA chart.
...
The Population Widget tells you how common a node is across demographic buckets. The Assembly Widget tells you which nodes a haplotype physically walks through. The PCA Widget sits between the two and answers a subtler question:

> *For a given haplotype, which pieces of this locus have ancestry calls, and where do those calls sit in PCA space?*

This is useful because a single haplotype can carry ancestry signal that varies along the locus — one stretch may cluster with African references, another with European. PCLAI captures that as a coordinate per node, and the PCA Widget makes the local-ancestry story directly visible: you pick a haplotype and immediately see which of its traversed nodes have ancestry information at all, and where each of those nodes lands in the 2D ancestry plane.
...
- **Not every haplotype-on-a-node has a PCLAI call.** A haplotype can walk through a node and still have an empty `pclai: []` array. The PCA Widget shows only haplotypes with *at least one* call; the Assembly Widget shows everyone.
- **A node can carry multiple PCLAI calls for the same haplotype** — different stretches of the haplotype's mapping to that node can land at different PCA coordinates with different percentages.
- **The `[x, y]` coordinates are ancestry components**, not genome coordinates. They live in the same 2D space as the HPRC reference PCA panel that backs the chart.
...
When you click a row in the PCA Widget, three coordinated things happen:

**1. The 3D graph redraws.** The widget fires a `pcaWidget:emphasis` event carrying the coordinate key, the set of nodes that carry a PCLAI call for that haplotype, and the "absent" node set. The node-emphasis look walks the graph and paints:
...
**2. The PCA Chart redraws.** The `PCAChartService` also subscribes to `pcaWidget:emphasis`. When it receives the event, it:

- Records the selected coordinate key.
- **Dims the reference PCA dots** — the background population drawn from `hprc-reference-pca.tsv`, which plots the HPRC reference panel in ancestry space — by converting them to grayscale and dropping their opacity. This pushes the reference panel into the background while keeping it as a spatial guide.
...
**3. The chart stays coupled to node hover.** Independently of the widget selection, hovering a node in the 3D graph publishes a `lineIntersection` event; the PCA Chart listens for this and plots *that node's* PCLAI coordinates. When a coordinate key is selected in the widget, the chart filters the hovered node's dots down to that haplotype. This is the mechanism that gives the widget its dual-view feel: the graph view and the chart view update together whenever either your selection or your hover changes.

## Relationship to the other widgets

- **vs. Assembly Widget.** Both list `assembly#haplotype` entries and both emphasize nodes on click, but they are reading different things. The Assembly Widget's list is every haplotype that traverses the locus (from `node.assembly[]`), and its emphasis is the full topological walk. The PCA Widget's list is only haplotypes with PCLAI calls (from `pclai_coordinates`), and its emphasis is the subset of nodes with a call — always a subset of the assembly walk, often a much smaller one. A haplotype can appear in the Assembly Widget but be absent from the PCA Widget.

- **vs. Population Widget.** The Population Widget summarizes the panel across demographic buckets; the PCA Widget drills into one individual haplotype's ancestry signal along the locus. They answer complementary questions: *"how does frequency vary across populations?"* versus *"how does ancestry vary along one haplotype?"*

- **Dataset applicability.** The PCA Widget is only useful on HPRC-format datasets. On a standard PGB dataset, `pclai_coordinates` is absent, the widget's launch button is disabled, and neither the widget nor the PCA Chart can be opened.

## What it is *not*

- **Not a whole-genome PCA.** The coordinates are *local* ancestry calls confined to the loaded locus. Cluster positions reflect the ancestry of a specific stretch of sequence, not the individual as a whole.
...
2. Open the PCA Widget and the PCA Chart from the navigation bar. The reference panel appears as a muted backdrop on the chart.
