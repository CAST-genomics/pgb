...
/**
 * Frame cost, sampled by the harness rather than read off the page.
 *
 * The spike's dev page carried an FPS meter and this read `.harness-fps` out of it. That
 * meter answered a question the spike closed and did not migrate (`devTubeMapRoute.ts` says
 * so in as many words), so the sampler is injected here instead. Same measurement — the
 * worst gap between consecutive animation frames — with the harness owning it, which is
 * better: nothing about what the numbers mean now depends on page chrome that a redesign
 * could take away.
 */
async function resetFrameMeter() {
    await page.evaluate(() => {
        ...
        state.handle = requestAnimationFrame(tick)
        state.stop = () => cancelAnimationFrame(state.handle)

        window.__frameMeter = state
    })
}

async function worstFrame() {
    return await page.evaluate(() => window.__frameMeter.worst)
}
